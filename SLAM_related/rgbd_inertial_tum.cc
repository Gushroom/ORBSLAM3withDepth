...
void LoadImagesTUMVI(const string &strPathLeft, const string &strPathDepth, const string &strPathTimes,
                vector<string> &vstrImageLeft, vector<string> &vstrImageDepth, vector<double> &vTimeStamps);
...
int main(int argc, char **argv)
{
    ...
    if(argc < 7) 
    {
        cerr << endl << "Usage: ./rgbd_inertial_tum path_to_vocabulary path_to_settings path_to_rgb_image_folder path_to_depth_image_folder path_to_times_file path_to_imu_data (trajectory_file_name)" << endl;
        return 1;
    }
    ...
    vector< vector<string> > vstrImageDepthFilenames;
    ...
    vstrImageDepthFilenames.resize(num_seq);
    ...
    for (seq = 0; seq<num_seq; seq++)
    {
        ...
        LoadImagesTUMVI(string(argv[4*(seq+1)-1]), string(argv[4*(seq+1)]), string(argv[4*(seq+1)+1]), vstrImageLeftFilenames[seq], vstrImageDepthFilenames[seq], vTimestampsCam[seq]);
    ...
    for (seq = 0; seq<num_seq; seq++)
    {
        ...
        for(int ni=0; ni<nImages[seq]; ni++, proccIm++)
        {
            ...
            imDepth = cv::imread(vstrImageDepthFilenames[seq][ni],cv::IMREAD_GRAYSCALE);
            ...
            clahe->apply(imDepth,imDepth);

            double tframe = vTimestampsCam[seq][ni];

            if(imLeft.empty() || imDepth.empty())
            {
            ...
            SLAM.TrackRGBD(imLeft,imDepth,tframe,vImuMeas);
                ...
                vector<string> &vstrImageLeft, vector<string> &vstrImageDepth, vector<double> &vTimeStamps)
{
    ...
    while(!fTimes.eof())
    {
        ...
        if(!s.empty())
        {
            ...
            vstrImageDepth.push_back(strPathDepth + "/" + item + ".png");
