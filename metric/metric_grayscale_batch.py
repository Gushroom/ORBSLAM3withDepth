...
depth_output_dir = '/root/autodl-tmp/cam0/depth/reverse-viridis-log'

os.makedirs(rgb_output_dir, exist_ok=True)
os.makedirs(depth_output_dir, exist_ok=True)

# Intrinsic parameters
fx, fy, cx, cy = 458.654, 457.296, 367.215, 248.375  # px
...
# print(f'min depth: {min_depth}')
    ...
    # Apply log transformation to the depth data
    log_depth = np.log(depth_data + 1)  # Adding 1 to avoid log(0)

    # Normalize the log-transformed depth data to [0, 1]
    log_normalized_depth = log_depth / np.max(log_depth)

    # Convert the normalized log-transformed depth data to a grayscale image
    log_normalized_depth_image = (log_normalized_depth * 255).astype(np.uint8)

    # Save the grayscale log image
    depth_save_path = os.path.join(depth_output_dir, os.path.splitext(image_name)[0] + '.png')
    cv2.imwrite(depth_save_path, log_normalized_depth_image)